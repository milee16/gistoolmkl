#' Gradient analysis of Roskilde Fjord
#'
...
gistoolRF_MM <- function() {
  ...
  WB_benthic_vs_WC_prod_plot <<- ggplot(data = WB_benthic_vs_WC_prod, aes(x = WB_gradient_no, y = prod_pct, group = interaction(scenario, prod_ID))) +
    ...
    scale_x_discrete(limits = c('0-6', '6-14', '14-18', '18-24'), expand = c(0,0.05)) +
  ...
  WB_monod_BC1_DIN + theme_pub() + theme(plot.margin = unit(c(15,1,1,1), "pt"), axis.title = element_text(size = 18), legend.justification = c(1,1.75),
                                         legend.direction = 'vertical',
                                         legend.box = 'vertical') +
    WB_monod_BC1_DIP + theme_pub() + theme(plot.margin = unit(c(15,1,1,1), "pt"), axis.title = element_text(size = 18), legend.justification = c(5.5,1.75),
                                           legend.direction = 'vertical',
                                           legend.box = 'vertical') +
  ...
  WB_AS_G1_DIP <<- ggplot(WB_G1_AS, aes(x = DIP, y = lower, group = scenario)) +
    ...
    scale_x_continuous(limits = c(0,0.6), breaks = seq(0,0.6, by = 0.1), labels = scales::number_format(accuracy = 0.1), position = 'top') +
    scale_y_continuous(limits = c(-8,0), breaks = seq(-8,0, by = 2), labels = scales::number_format(accuracy = 1))
